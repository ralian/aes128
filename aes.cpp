...
inline a128 shift_rows(a128 input) {
	register uint32_t output[4];
	//for (int i = 0; i < 4; i++)
	//	output[i] = 
...
inline a128 mix_cols(a128 input) {
...
inline uint64_t sub_bytes(uint64_t input) {
	...
	for (int i = 0; i < 8; i++) {		// Iterate over 4 cols and 2 rows		
		//input >>= (8*i+32*j);			// This is pseudo-recursive: we always assume active_bit is at the front of the vector.
